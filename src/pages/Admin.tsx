      ...
      `} style={{ background: colors.sidebarBg, borderColor: colors.sidebarBorder }}>
        {isMobile && (
          <div className="flex justify-end p-4">
            <button onClick={() => setSidebarOpen(false)} className="p-2" title="Fechar menu">
          ...
          {navItems.map(item => (
            <button
              ...
              style={section === item.id
                ? { background: `${ACCENT.blue}18`, border: `1px solid ${ACCENT.blue}33`, color: colors.sidebarTextActive }
        ...
        <div className="p-4 border-t" style={{ borderColor: colors.sidebarBorder }}>
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white text-xs font-bold"
              style={{ background: `linear-gradient(135deg, ${ACCENT.blue}, hsl(220, 70%, 60%))` }}>
          ...
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs uppercase"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 flex items-center justify-between px-4 md:px-6 border-b"
          ...
          <div className="flex items-center gap-3">
            {isMobile && (
              <button onClick={() => setSidebarOpen(true)} className="p-2" title="Abrir menu" style={{ color: colors.textPrimary }}>
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-6">
          {section === 'dashboard' && (
            <DashboardSection apiFetch={apiFetch} colors={colors} toastError={toastError}
              accentBlue={ACCENT.blue} accentOrange={ACCENT.orange} />
          )}
          {section === 'courses' && (
            <CursosSection apiFetch={apiFetch} colors={colors} toastSuccess={toastSuccess}
              toastError={toastError} accentBlue={ACCENT.blue} />
          )}
          {section === 'users' && (
            <UsuariosSection apiFetch={apiFetch} colors={colors} toastSuccess={toastSuccess}
              toastError={toastError} accentBlue={ACCENT.blue} accentOrange={ACCENT.orange} />
          )}
          {section === 'validation' && (
            <ValidacaoSection apiFetch={apiFetch} colors={colors} toastSuccess={toastSuccess}
              toastError={toastError} accentBlue={ACCENT.blue} accentOrange={ACCENT.orange} accentGreen={ACCENT.green} />
          )}
          {section === 'rules' && (
            <RegrasSection apiFetch={apiFetch} colors={colors} toastSuccess={toastSuccess}
              toastError={toastError} accentBlue={ACCENT.blue} accentOrange={ACCENT.orange} />
          )}
          {section === 'coordinators' && (
            <CoordenadoresSection apiFetch={apiFetch} colors={colors} toastSuccess={toastSuccess}
              toastError={toastError} accentBlue={ACCENT.blue} accentOrange={ACCENT.orange} />
          )}
          {section === 'settings' && (
            <ConfiguracoesSection apiFetch={apiFetch} colors={colors} toastSuccess={toastSuccess}
              toastError={toastError} accentBlue={ACCENT.blue} accentOrange={ACCENT.orange} />
          )}
        </main>
